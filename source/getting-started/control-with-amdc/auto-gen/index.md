...
After code generation, Simulink creates a folder (typically named `modelName_ert_rtw`) that contains all generated source and header files. To keep them separate from hand-written code for clarity and maintainability, all autogenerated `.c` and `.h` files generated files should be placed within the application directory in the AMDC repository. For example:
